...
app.get('/player_info/select_player', function(req, res) {
  ...
  var IDquery = 'SELECT id, name FROM football_players;';
  var Infoquery = 'SELECT major, year, rushing_yards, passing_yards, receiving_yards, img_src FROM football_players where football_players.id = ' + playerId + ';';
  var Gamequery = 'select count(*) from football_players INNER JOIN football_games ON football_players.id = ANY(players) WHERE football_players.id = ' + playerId + ';';
  db.task('get-everything', task => {
      return task.batch([
          task.any(IDquery),
          task.any(Infoquery),
